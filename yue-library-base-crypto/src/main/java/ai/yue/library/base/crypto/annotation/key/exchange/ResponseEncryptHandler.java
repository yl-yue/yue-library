	...
	public T beforeBodyWrite(T body, MethodParameter returnType, MediaType selectedContentType,
		...
		if (enableExchangeKeyEncrypt == true) {
			...
			if (methodAnnotation.useAuthTokenGetExchangeKey()) {
				...
				keyExchangeStorageKey = StrUtil.isBlank(StrUtil.subAfter(authorization, "Bearer ", false))
						? servletRequest.getParameter("access_token")
						: StrUtil.subAfter(authorization, "Bearer ", false);

				keyExchangeStorageKey = StrUtil.isBlank(servletRequest.getHeader("accessToken")) ? keyExchangeStorageKey
						: servletRequest.getHeader("accessToken");
			} else {
				...
				keyExchangeStorageKey = ((ServletServerHttpRequest) request).getServletRequest().getHeader(headerNameGetExchangeKey);
				if (StrUtil.isBlank(keyExchangeStorageKey)) {
					keyExchangeStorageKey = ((ServletServerHttpRequest) request).getServletRequest().getParameter(paramNameGetExchangeKey);
				}
			}
