...
### 四、校验异常处理
当校验不通过时会抛出`ValidateException`，bl默认已在`ResultExceptionHandler`类中进行了拦截处理，将异常提示转换成友好的RESTful响应。
...
- 关于如何自定义校验注解，参考`@Cellphone`、`@IdCard`等（即：bl扩展实现）校验注解即可