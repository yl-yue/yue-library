...
##### Spring 定时任务中，`@Lock` 注解失效
Spring AOP 的实现机制，只有通过 Spring 容器获取的 Bean，且由 Spring AOP 代理的对象，方法调用才会被切面拦截。

`@Scheduled` 注解的方法由 Spring 的调度器直接调用，并不是通过代理对象调用。因此，AOP 切面（MethodInterceptor）不会拦截 `@Scheduled` 方法的执行。
...
public class ScheduledTask {
    ...
    public void scheduledMethod() {
        // 通过 ApplicationContext 获取代理对象，再调用需要加锁执行的方法
        SpringUtils.getBean(ScheduledTask.class).lockedMethod();
    }
