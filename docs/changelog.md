...
---
### 版本说明
　　bl的版本命名方式，继2.1.0开始采用与 [SpringBoot版本发行](https://github.com/spring-projects/spring-boot/wiki/Supported-Versions) 对应的命名方式。<br>
　　`bl-base`为其他模块的基础依赖（简称基础库），所以若需要引入除基础库之外的模块（如：web、webflux、data-jdbc、data-redis），可以不引入`bl-base`。
...
[👉点击查看pom.xml依赖](https://gitee.com/yl-yue/bl/blob/master/pom.xml)

### j11.2.6.3【规划中】
### j11.2.6.2【2023-03-28】
1. 移除bl-pay模块，放至里程碑实现
2. 移除bl-data-jdbc模块，放至里程碑实现
3. 移除bl-web-grpc模块，放至里程碑实现
4. 新增bl-data-mybatis模版
...
#### Maven关键依赖库
[**👉Maven详细依赖定义见pom.xml文件**](https://gitee.com/yl-yue/bl/blob/j11.2.6.2/pom.xml)
...
这是一个分水岭版本，从下本版本开始，bl将不再注重大而全，而是精简与引领技术栈，会封装常用特性，但不会引入偏门的技术栈，如：grpc。
此类特性会作为脚手架demo提供。
- 新增grpc模块，但下个版本会移除，变为脚手架demo方式
- 完善了jdbc框架，提供了优雅SQL编写等诸多能力，但很遗憾下个版本此模块将被移除，以后会改为支持mybatis-plus
- 删除bl-dependencies模块（长期无多大价值），改直接依赖父级bl模块
...
#### Maven关键依赖库
[**👉Maven详细依赖定义见pom.xml文件**](https://gitee.com/yl-yue/bl/blob/j11.2.6.1/pom.xml)
...
#### Bug修复
- 【template-boot】修正因SpringBoot2.4版本新的配置文件机制，导致的启动失败 [#I40ONA](https://gitee.com/yl-yue/bl/issues/I40ONA)

#### Maven关键依赖库
[**👉Maven详细依赖定义见pom.xml文件**](https://gitee.com/yl-yue/bl/blob/j11.2.6.0/pom.xml)
...
#### Bug修复
- 【web】解决SpringBoot2.4版本新出现的跨域问题 [#I3OV7B](https://gitee.com/yl-yue/bl/issues/I3OV7B)
- 【web】修复异步线程装饰器在开启ServletAsyncContext时，接口响应被无故追加404异常 [#I3HTAW](https://gitee.com/yl-yue/bl/issues/I3HTAW)

#### Maven仓库实际发布版本号
`j8.2.4.0`、`j11.2.4.0`

[**关键pom.xml依赖：**](https://gitee.com/yl-yue/bl/blob/j11.2.4.0/pom.xml)
...
[**关键pom.xml依赖：**](https://gitee.com/yl-yue/bl/blob/j11.2.3.3/pom.xml)
...
- 【base】MapUtils增强值提取，支持list根据key提取map提取值支持map、fastjson [pulls !17](https://gitee.com/yl-yue/bl/pulls/17)
...
[**关键pom.xml依赖：**](https://gitee.com/yl-yue/bl/blob/j11.2.3.2/pom.xml)
...
#### Bug修复
- 【web】解决@RequestMapping中指定produces为xml类型时，JavaBean转换会去解析xml内容BUG [#I2ALJW](https://gitee.com/yl-yue/bl/issues/I2ALJW)
...
[**关键pom.xml依赖：**](https://gitee.com/yl-yue/bl/blob/j11.2.3.1/pom.xml)
...
- 【web】HttpMessageConverter选用fastjson时，解析非json格式响应体错误 [#I2ALJW](https://gitee.com/yl-yue/bl/issues/I2ALJW)

#### Maven仓库实际发布版本号
`j8.2.3.0`、`j11.2.3.0`

[**关键pom.xml依赖：**](https://gitee.com/yl-yue/bl/blob/j11.2.3.0/pom.xml)
...
[点击查看更多依赖版本定义](https://gitee.com/yl-yue/bl/blob/master/pom.xml)
...
#### 历史版本命名说明
bl的版本命名方式，采用SpringCloud版本名作为前缀，然后以.1、.2、.3...这种形式，目的是为了方便区分基础依赖版本。<br>
...
- 新增 `bl-base-crypto` 模块，基于hutool提供更完善的加解密、签名等操作。提供自动配置全局单例
- 新增 `bl-pay` 模块，基于pay-java-parent进行二次封装，**让你真正做到一行代码实现支付聚合**，让你可以不用理解支付怎么对接，只需要专注你的业务
- 优化serialVersionUID
...
　　2019年初项目进行开源迭代直到正式发布第一个Maven版本：[Finchley.SR2.SR1](https://search.maven.org/artifact/ai.ylyue/bl-dependencies/Finchley.SR2.SR1/pom)
